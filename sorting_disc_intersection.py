...
# how many times do the discs intersect

class DiscLog():
    def __init__(self, x, start_end): # x is where it is happening, start or end of teh disk
        self.x = x
        self.start_end = start_end

def solution(A):
    discHistory = [] # define an empty array, this will be the event log
    for i in range(len(A)):
        discHistory.append(DiscLog(i - A[i], 1)) # the start of the disk
        discHistory.append(DiscLog(i + A[i], -1)) # the end of the disk
    discHistory.sort(key=lambda d: (d.x, -d.start_end)) #then sort the disk history, use lamda to know how to sort
    intersections = 0 #this will hold teh result
    active_intersections = 0 # this will be the cumulative number
